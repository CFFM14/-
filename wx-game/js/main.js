...
var btnRects=[];
var editTubes=[],editCaps=[],editHidden=[],editActiveColor=0;

// ── 难度配置 ──
var DIFFS=[
  {n:5,e:2,sh:60,label:'⭐ 简单',hint:'5色·2空管'},{n:6,e:2,sh:80,label:'⭐⭐ 中等',hint:'6色·2空管'},
  {n:7,e:2,sh:120,label:'⭐⭐⭐ 困难',hint:'7色·2空管'},{n:8,e:2,sh:160,label:'🌟🌟 专家',hint:'8色·2空管'},
  {n:10,e:2,sh:200,label:'💀 地狱',hint:'10色·2空管'}
];
var VC_DIFFS=[{n:3,label:'🌱 入门',hint:'3色·容量4~5'},{n:4,label:'🌿 初级',hint:'4色·容量4~6'},{n:5,label:'🌳 中级',hint:'5色·容量4~6'},{n:6,label:'🏔 高级',hint:'6色·容量4~7'}];
var BB_DIFFS=[{n:5,label:'🎁 简单',hint:'5色·盲盒',mxH:1,tr:0.25},{n:6,label:'🎁 中等',hint:'6色·盲盒',mxH:2,tr:0.35},{n:7,label:'🎁 困难',hint:'7色·盲盒',mxH:2,tr:0.5},{n:8,label:'🎁 专家',hint:'8色·盲盒',mxH:2,tr:0.6}];
...
function checkSolvable(tubes,md){md=md||600;
  ...
  var vis=new Set();function key(s){var nm=s.map(function(t){return t.join(',');});var em=[],pu=[],re=[];for(var i=0;i<nm.length;i++){var t=nm[i];var ly=t.split(',');if(ly.every(function(c){return c==='transparent';}))em.push(t);else if(ly.every(function(c){return c===ly[0];}))pu.push(t);else re.push(t);}return re.sort().concat(pu.sort()).concat(em.sort()).join('|');}
  function dfs(s,d){if(d>md)return false;if(s.every(_pu))return true;var k=key(s);if(vis.has(k))return false;vis.add(k);
    for(var fr=0;fr<s.length;fr++){if(_pu(s[fr]))continue;var st=_tc(s[fr]);if(st==='transparent')continue;
      for(var to=0;to<s.length;to++){if(fr===to)continue;var dt=_tc(s[to]);if(dt!=='transparent'&&dt!==st)continue;if(_sp(s[to])===0)continue;
        ...
        var ad=0;for(var i=0;i<s[to].length&&ad<cn;i++)if(s[to][i]==='transparent'){s[to][i]=st;ad++;}if(dfs(s,d+1))return true;s[fr]=sf;s[to]=st2;}}return false;}
...
function drawTube(x,y,water,sel,fbKind,hidden,rv,rvPrev,cap,tubeIdx){cap=cap||4;var sh=straightH(cap),th=totalH(cap);var cx=x+TW/2,bodyTop=y+RIM_H,sBot=bodyTop+sh;var innerX=x+WALL,innerTopY=bodyTop+WALL;ctx.save();if(sel){var glow=ctx.createRadialGradient(cx,y+th/2,TW*0.3,cx,y+th/2,TW*1.5);glow.addColorStop(0,'rgba(200,170,110,0.22)');glow.addColorStop(1,'rgba(200,170,110,0)');ctx.fillStyle=glow;ctx.fillRect(x-24,y-8,TW+48,th+20);}var tp=function(){ctx.beginPath();ctx.moveTo(x,bodyTop);ctx.lineTo(x,sBot);ctx.arc(cx,sBot,BOTTOM_R,Math.PI,0,true);ctx.lineTo(x+TW,bodyTop);ctx.closePath();};tp();ctx.fillStyle='rgba(248,246,243,0.55)';ctx.fill();tp();ctx.strokeStyle='rgba(175,170,162,0.50)';ctx.lineWidth=1.8;ctx.stroke();ctx.save();ctx.beginPath();ctx.moveTo(innerX,innerTopY);ctx.lineTo(innerX,sBot);ctx.arc(cx,sBot,INNER_R,Math.PI,0,true);ctx.lineTo(innerX+IW,innerTopY);ctx.closePath();ctx.clip();if(fbKind){ctx.fillStyle=fbKind==='reject'?'rgba(255,120,120,0.1)':'rgba(120,255,120,0.1)';ctx.fillRect(innerX,innerTopY,IW,sBot+INNER_R-innerTopY);}var blocks=[];for(var bi=0;bi<water.length;bi++){if(water[bi]==='transparent')continue;var bc=water[bi];if(blocks.length>0&&blocks[blocks.length-1].c===bc)blocks[blocks.length-1].n++;else blocks.push({c:bc,n:1});}var totalL=0;for(var bj=0;bj<blocks.length;bj++)totalL+=blocks[bj].n;var innDp=sBot+INNER_R;var ts=now()/1000;var drawIdx=0;for(var bi2=0;bi2<blocks.length;bi2++){var clr=blocks[bi2].c,cnt=blocks[bi2].n;var ly=innDp-(drawIdx+cnt)*LAYER_H,lh=cnt*LAYER_H+1;if(water._partialIdx>=0&&water._partialIdx>=drawIdx&&water._partialIdx<drawIdx+cnt){lh-=(1-water._partialFrac)*LAYER_H;ly+=(1-water._partialFrac)*LAYER_H;}var isHid=hidden>0&&drawIdx<hidden;var isTop=(bi2===blocks.length-1);if(isHid){var g=ctx.createLinearGradient(innerX,ly,innerX+IW,ly+lh);var h0=((ts*50+ly*3)%360);g.addColorStop(0,'hsl('+h0+',80%,45%)');g.addColorStop(0.25,'hsl('+((h0+60)%360)+',85%,42%)');g.addColorStop(0.5,'hsl('+((h0+120)%360)+',80%,48%)');g.addColorStop(0.75,'hsl('+((h0+180)%360)+',85%,40%)');g.addColorStop(1,'hsl('+((h0+240)%360)+',80%,46%)');ctx.fillStyle=g;ctx.fillRect(innerX,ly,IW,lh);ctx.fillStyle='rgba(255,255,255,0.8)';ctx.font='bold 16px sans-serif';ctx.textAlign='center';ctx.fillText('?',cx,ly+lh/2+5);}else{var g2=ctx.createLinearGradient(0,ly,0,ly+lh);g2.addColorStop(0,lighten(clr,20));g2.addColorStop(0.55,clr);g2.addColorStop(1,darken(clr,12));ctx.fillStyle=g2;ctx.fillRect(innerX,ly,IW,lh);if(isTop){var sY2=ly;var nw2=now()/1000;ctx.strokeStyle='rgba(255,255,255,0.40)';ctx.lineWidth=1.8;var wa=1.5,sx2=innerX+3,ex2=innerX+IW-3;ctx.beginPath();ctx.moveTo(sx2,sY2+Math.sin(nw2*3+sx2*0.04)*wa);for(var wx=sx2+2;wx<=ex2;wx+=2)ctx.lineTo(wx,sY2+Math.sin(nw2*3+wx*0.04)*wa);ctx.stroke();}}drawIdx+=cnt;}ctx.restore();ctx.restore();}
...
function render(nTs){
  ...
  for(var ai=transferAnims.length-1;ai>=0;ai--){var a=transferAnims[ai];var dt2=nTs-(a._last||nTs);if(dt2<0)dt2=0;if(dt2>33)dt2=33;a.elapsed=Math.min(a.duration,a.elapsed+dt2);a._last=nTs;var at2=Math.min(a.elapsed/a.duration,1);
  ...
  var fbTube=-1,fbKind=null;if(feedback){var age=nTs-feedback.t;if(age<FB_DURATION){fbTube=feedback.tube;fbKind=feedback.kind;}else feedback=null;}
...
// ── UI ──
function drawBtn(x,y,w,h,txt,disabled){ctx.fillStyle='#fff';ctx.strokeStyle=disabled?'#ddd':'#e2ded8';ctx.lineWidth=1.5;rr(x,y,w,h,22);ctx.fill();ctx.stroke();ctx.fillStyle=disabled?'#ccc':'#3b3b3b';ctx.font='14px sans-serif';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(txt,x+w/2,y+h/2);}
...
function drawUI(){
  ...
  if(screen==='menu'){
    ...
    for(var i=0;i<btns.length;i++){var by=sy+i*(bh+gp);drawBtn((sw-bw)/2,by,bw,bh,btns[i].txt);btnRects.push({id:btns[i].id,x:(sw-bw)/2,y:by,w:bw,h:bh});}
  }else if(screen==='game'){
    var bw2=64,bh2=36,gp2=6;var btns2=[
      {id:'undo',txt:'↩',disabled:undoStack.length===0||won||transferAnims.length>0},
      {id:'hint',txt:hintThinking?'💭':'💡',disabled:won||transferAnims.length>0||hintThinking},
      {id:'reset',txt:'↻',disabled:transferAnims.length>0},{id:'addtube',txt:'＋',disabled:won||transferAnims.length>0},{id:'back',txt:'←',disabled:transferAnims.length>0}
    ];var totW=btns2.length*bw2+(btns2.length-1)*gp2;var sx=(sw-totW)/2,gy=sh-bh2-14;
    for(var i=0;i<btns2.length;i++){drawBtn(sx,gy,bw2,bh2,btns2[i].txt,btns2[i].disabled);btnRects.push({id:btns2[i].id,x:sx,y:gy,w:bw2,h:bh2});sx+=bw2+gp2;}
    ...
    var bw3=300,bh3=38,sy3=72,gp3=6;
    for(var i=0;i<DIFFS.length;i++){var by3=sy3+i*(bh3+gp3);drawBtn((sw-bw3)/2,by3,bw3,bh3,DIFFS[i].label+'    '+DIFFS[i].hint);btnRects.push({id:'diff_'+i,x:(sw-bw3)/2,y:by3,w:bw3,h:bh3,di:i});}
    drawBtn(10,10,50,30,'←');btnRects.push({id:'back',x:10,y:10,w:50,h:30});
    ...
    for(var i=0;i<BB_DIFFS.length;i++){var by4=sy4+i*(bh4+gp4);drawBtn((sw-bw4)/2,by4,bw4,bh4,BB_DIFFS[i].label+'  '+BB_DIFFS[i].hint);btnRects.push({id:'bb_'+i,x:(sw-bw4)/2,y:by4,w:bw4,h:bh4,bi:i});}
    sy4+=BB_DIFFS.length*(bh4+gp4)+10;ctx.fillText('🧪 变容模式',sw/2,sy4);sy4+=20;
    for(var i=0;i<VC_DIFFS.length;i++){var by5=sy4+i*(bh4+gp4);drawBtn((sw-bw4)/2,by5,bw4,bh4,VC_DIFFS[i].label+'  '+VC_DIFFS[i].hint);btnRects.push({id:'vc_'+i,x:(sw-bw4)/2,y:by5,w:bw4,h:bh4,vi:i});}
    drawBtn(10,10,50,30,'←');btnRects.push({id:'back',x:10,y:10,w:50,h:30});
    ...
    var etY=95,eg=8;for(var ti=0;ti<editTubes.length;ti++){var etX=20+ti*(TW+eg);var sh2=straightH(editCaps[ti]||4),th2=totalH(editCaps[ti]||4);var cx3=etX+TW/2,boTop=etY+RIM_H,sb2=boTop+sh2;
      ...
      var rev=[].concat(editTubes[ti]).reverse();for(var ri=0;ri<rev.length;ri++){var cy2=sb2+INNER_R-(ri+1)*LAYER_H;ctx.fillStyle=(rev[ri]==='transparent'?'#f5f3f0':rev[ri]);ctx.fillRect(etX+WALL,cy2,IW,LAYER_H);if(ri===rev.length-1&&rev[ri]!=='transparent'){ctx.strokeStyle='rgba(255,255,255,0.4)';ctx.lineWidth=1.5;ctx.beginPath();ctx.moveTo(etX+WALL,cy2+2);ctx.lineTo(etX+IW,cy2+2);ctx.stroke();}}ctx.restore();
      ctx.fillStyle='#888';ctx.font='10px sans-serif';ctx.textAlign='center';ctx.fillText(editCaps[ti]+'格',etX+TW/2,etY+th2+12);
      drawBtn(etX-2,etY+th2+16,24,20,'-');btnRects.push({id:'ecapm_'+ti,x:etX-2,y:etY+th2+16,w:24,h:20,ti:ti});drawBtn(etX+TW-22,etY+th2+16,24,20,'+');btnRects.push({id:'ecapp_'+ti,x:etX+TW-22,y:etY+th2+16,w:24,h:20,ti:ti});
      btnRects.push({id:'etube_'+ti,x:etX+WALL,y:boTop,w:IW,h:sh2+INNER_R-WALL,ti:ti});}
    var eby=etY+totalH(4)+60;drawBtn(20,eby,60,34,'＋管');btnRects.push({id:'eadd',x:20,y:eby,w:60,h:34});drawBtn(90,eby,60,34,'－管');btnRects.push({id:'erm',x:90,y:eby,w:60,h:34});drawBtn(160,eby,60,34,'💾保存');btnRects.push({id:'esave',x:160,y:eby,w:60,h:34});drawBtn(230,eby,60,34,'▶试玩');btnRects.push({id:'etest',x:230,y:eby,w:60,h:34});
    drawBtn(10,6,50,30,'←');btnRects.push({id:'back',x:10,y:6,w:50,h:30});
    ...
    var lvs=loadLevels();if(lvs.length===0){ctx.font='15px sans-serif';ctx.fillStyle='#999';ctx.textAlign='center';ctx.fillText('还没有自制关卡',sw/2,sh/2);}else{
      for(var i=0;i<Math.min(lvs.length,10);i++){var lv=lvs[i];var ly2=70+i*50;ctx.fillStyle='#fff';ctx.strokeStyle='#ddd';ctx.lineWidth=1;rr(20,ly2,sw-40,42,10);ctx.fill();ctx.stroke();ctx.fillStyle='#333';ctx.font='14px sans-serif';ctx.textAlign='left';ctx.fillText(lv.name||'关卡',30,ly2+18);ctx.fillStyle='#999';ctx.font='11px sans-serif';ctx.fillText((lv.colorCount||0)+'色 · '+(lv.tubes?lv.tubes.length:0)+'管',30,ly2+34);drawBtn(sw-70,ly2+6,50,28,'▶');btnRects.push({id:'playlv_'+i,x:sw-70,y:ly2+6,w:50,h:28,li:i});}
    }drawBtn(10,6,50,30,'←');btnRects.push({id:'back',x:10,y:6,w:50,h:30});
  }else if(screen==='help'){
    ctx.fillStyle='#3b3b3b';ctx.font='bold 20px sans-serif';ctx.textAlign='center';ctx.fillText('📖 游戏说明',sw/2,30);
    var lines=['目标：同色水集中到同一试管','规则：点击选管再点另一根倒水','只能倒空管或同色水上方','每管最多装对应容量的水','撤销/重置/加管/返回 按钮在底部','💡提示按钮自动执行一步倒水','按H键也可触发提示'];
...
// ── 编辑器 ──
function initEditor(){editTubes=[];for(var i=0;i<3;i++)editTubes.push(['transparent','transparent','transparent','transparent']);editCaps=[4,4,4];editHidden=[0,0,0];editActiveColor=0;screen='editor';layout();}
...
function saveLevel(){var cs=new Set();var he=false;editTubes.forEach(function(t){var f=0;t.forEach(function(c){if(c!=='transparent')cs.add(c);else f++;});if(f>0)he=true;});if(cs.size<2){wx.showModal({title:'',content:'请至少使用2种颜色！',showCancel:false});return;}if(!he){wx.showModal({title:'',content:'请添加至少一根空管！',showCancel:false});return;}var data=editTubes.map(function(t){return[].concat(t);});
  function doSave(){var lvs=loadLevels();var id=Date.now().toString(36);lvs.push({id:id,name:'自制关卡',tubes:data,caps:editCaps,hidden:editHidden,colorCount:cs.size,createdAt:Date.now()});saveLevels(lvs);wx.showModal({title:'',content:'已保存！',showCancel:false});}
...
function testLevel(){var cs=new Set();var he=false;editTubes.forEach(function(t){var f=0;t.forEach(function(c){if(c!=='transparent')cs.add(c);else f++;});if(f>0)he=true;});if(cs.size<2){wx.showModal({title:'',content:'请至少使用2种颜色！',showCancel:false});return;}if(!he){wx.showModal({title:'',content:'请添加至少一根空管！',showCancel:false});return;}var data=editTubes.map(function(t){return[].concat(t);});
  function doTest(){playMode='custom';playLevelId=null;playBack='editor';resetFromData(data,editHidden,editCaps);}
...
canvas.addEventListener('touchstart',function(e){var p=getPos(e),b=hitBtn(p);if(b){handleBtn(b,p);return;}if(screen==='game')handleGameTap(p);});

function handleBtn(b,p){
  if(!b.id)return;if(b.id==='back'){screen='menu';layout();return;}if(b.id==='challenge'){screen='challenge';layout();return;}if(b.id==='special'){screen='special';layout();return;}if(b.id==='editor'){initEditor();return;}if(b.id==='mylevels'){screen='mylevels';layout();return;}if(b.id==='help'){screen='help';layout();return;}if(b.id==='undo'){undo();return;}if(b.id==='hint'){triggerHint();return;}if(b.id==='reset'){restartLevel();return;}if(b.id==='addtube'){addTubeDuringPlay();return;}
  if(b.id.indexOf('diff_')===0){var d=DIFFS[b.di];if(d){playMode='challenge';playLevelId=null;resetFromData(genLevelSmart(d.n,d.e,d.sh));}return;}
  if(b.id.indexOf('bb_')===0){var d2=BB_DIFFS[b.bi];if(d2){var bm=genBlindbox(d2.n,d2.mxH,d2.tr);playMode='blindbox';resetFromData(bm.data,bm.mask);}return;}
  if(b.id.indexOf('vc_')===0){var d3=VC_DIFFS[b.vi];if(d3){var vg=genVaricapLevel(d3.n);playMode='varicap';resetFromData(vg.tubes,null,vg.caps);}return;}
  if(b.id.indexOf('ec_')===0){if(b.id==='ec_erase'){editActiveColor=-1;return;}editActiveColor=b.ci;return;}
  if(b.id.indexOf('etube_')===0){var ti=b.ti;var boTop2=95+RIM_H;var cap2=editCaps[ti]||4;var sh3=straightH(cap2);var innDp2=(boTop2+sh3)+INNER_R;var relY=p.y-(95+RIM_H);for(var ri=0;ri<editTubes[ti].length;ri++){var cy3=innDp2-(ri+1)*LAYER_H;if(p.y>=cy3&&p.y<=cy3+LAYER_H){var li=editTubes[ti].length-1-ri;editTubes[ti][li]=editActiveColor===-1?'transparent':PALETTE[editActiveColor]||PALETTE[0];break;}}return;}
  if(b.id.indexOf('ecapm_')===0){changeCap(b.ti,-1);return;}if(b.id.indexOf('ecapp_')===0){changeCap(b.ti,1);return;}if(b.id==='eadd'){addEditTube();return;}if(b.id==='erm'){rmEditTube();return;}if(b.id==='esave'){saveLevel();return;}if(b.id==='etest'){testLevel();return;}
  if(b.id.indexOf('playlv_')===0){var lvs2=loadLevels();var lv=lvs2[b.li];if(lv){playMode='custom';playLevelId=lv.id;playBack='mylevels';resetFromData(lv.tubes.map(function(t){return[].concat(t);}),lv.hidden||null,lv.caps||null);}return;}
}
...
function handleGameTap(p){
  for(var i=0;i<slots.length;i++){var s=slots[i],cap=tubeCaps[i]||4;if(p.x>=s.x&&p.x<=s.x+TW&&p.y>=s.y&&p.y<=s.y+totalH(cap)){
    ...
    else if(selected===i){selected=-1;}else{var from=selected,to=i;selected=-1;if(isAnimating(to)||isCapped(to))return;attempt(from,to);}return;}}
}

// ── 主循环 ──
function gameLoop(){var nt=now();if(screen==='game')render(nt);else{ctx.clearRect(0,0,sw,sh);drawBg();drawUI();}}
